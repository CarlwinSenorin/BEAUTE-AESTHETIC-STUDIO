...
<div class="staff-card">
    ...
    <form method="POST" class="staff-form">
        ...
        <div class="form-group" style="margin-bottom: 15px;">
            ...
            <input type="text" name="name" value="<?php echo htmlspecialchars($edit_package['name'] ?? ''); ?>" required class="form-control">
        </div>
        
        <div class="form-group" style="margin-bottom: 15px;">
            <label>Description</label>
            <textarea name="description" rows="3" class="form-control"><?php echo htmlspecialchars($edit_package['description'] ?? ''); ?></textarea>
        </div>
        
        <div class="form-group" style="margin-bottom: 15px;">
            <label>Number of Persons (Pax)</label>
            <input type="number" name="pax" value="<?php echo $edit_package['pax'] ?? 1; ?>" min="1" max="10" required class="form-control">
            <small style="color:#888;">How many persons this package is for</small>
        </div>
        
        <div class="form-group" style="margin-bottom: 15px;">
            <label>Select Services</label>
            <div class="services-wrapper" style="border: 1px solid #ddd; padding: 1rem; border-radius: 8px; max-height: 400px; overflow-y: auto; background:#fff;">
                <?php 
                ...
                ?>
                    <div class="category-group" style="margin-bottom: 1rem;">
                        <h4 style="margin: 0.5rem 0; color: #2a9d8f; border-bottom: 1px solid #eee; padding-bottom: 0.25rem; font-size:1rem;">
                            <?php echo htmlspecialchars(ucfirst($current_category)); ?>
                        </h4>
                        <div class="services-grid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(180px, 1fr)); gap: 0.5rem;">
                    <?php endif; ?>
                    
                    <label style="display: flex; align-items: center; gap: 0.5rem; background: #f8f9fa; padding: 0.5rem; border-radius: 4px; cursor:pointer; font-size:0.9rem;">
                        ...
                        <div>
                            ...
                            <div style="font-size: 0.75rem; color: #666;">₱<?php echo number_format($service['price'], 2); ?></div>
                        </div>
                    </label>
        ...
        <div style="display:grid; grid-template-columns: 1fr 1fr 1fr; gap: 15px; margin-bottom: 15px;">
            <div class="form-group">
                ...
                <input type="number" id="original_price" name="original_price" value="<?php echo $edit_package['original_price'] ?? 0; ?>" min="0" step="0.01" readonly style="background-color: #e9ecef;" class="form-control">
                <small style="color:#888;">Auto-calculated</small>
            </div>
             <div class="form-group">
                <label>Discount Percentage (%)</label>
                <input type="number" id="discount_percentage" name="discount_percentage" value="<?php echo $edit_package['discount_percentage'] ?? 0; ?>" min="0" max="100" step="0.1" class="form-control">
            </div>
            <div class="form-group">
                <label>Discounted Price (₱)</label>
                <input type="number" id="discounted_price" name="discounted_price" value="<?php echo $edit_package['discounted_price'] ?? 0; ?>" min="0" step="0.01" required class="form-control">
            </div>
        </div>
        ...
        <div style="display:grid; grid-template-columns: 1fr 1fr 1fr; gap: 15px; margin-bottom: 15px;">
            <div class="form-group">
                ...
                <input type="date" name="valid_from" value="<?php echo $edit_package['valid_from'] ?? date('Y-m-d'); ?>" required class="form-control">
            </div>
            <div class="form-group">
                <label>Valid Until (Optional)</label>
                <input type="date" name="valid_until" value="<?php echo $edit_package['valid_until'] ?? ''; ?>" class="form-control">
            </div>
            <div class="form-group">
                <label>Status</label>
